...
export default {async fetch(req,env){const u=new URL(req.url),token=req.headers.get('Authorization')?.replace('Bearer ','')||u.searchParams.get('token')||'';if(u.pathname.startsWith('/api/auth/'))return env.USERS.get(env.USERS.idFromName('global')).fetch(req);if(u.pathname==='/api/turn'){const user=await identity(env,token);if(!user)return j({error:'Não autenticado'},401);if(!env.METERED_TURN_API_KEY)return j({error:'TURN ainda não configurado.'},503);const upstream=await fetch(`https://vixvoice.metered.live/api/v1/turn/credential?secretKey=${encodeURIComponent(env.METERED_TURN_API_KEY)}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({expiryInSeconds:3600,label:`vix-${user.id}`})});if(!upstream.ok)return j({error:'Não foi possível obter credenciais TURN.'},502);const data=await upstream.json(),auth={username:data.username,credential:data.password};return j([{urls:'stun:stun.relay.metered.ca:80'},{urls:'turn:global.relay.metered.ca:80',...auth},{urls:'turn:global.relay.metered.ca:80?transport=tcp',...auth},{urls:'turn:global.relay.metered.ca:443',...auth},{urls:'turns:global.relay.metered.ca:443?transport=tcp',...auth}])}if(u.pathname.startsWith('/api/servers')){u.searchParams.set('token',token);return env.SERVERS.get(env.SERVERS.idFromName('global')).fetch(new Request(`https://servers${u.pathname}${u.search}`,req))}const m=u.pathname.match(/^\/signal\/([\w-]+)$/);if(m)return env.SERVERS.get(env.SERVERS.idFromName('global')).fetch(new Request(`https://servers/ws/${m[1]}?token=${encodeURIComponent(token)}`,req));return env.ASSETS.fetch(req)}};
