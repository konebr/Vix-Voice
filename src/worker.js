...
Servers.prototype.fetch=async function(req){const u=new URL(req.url),user=await this.user(req);if(!user)return j({error:'Não autenticado'},401);this.c.storage.sql.exec('CREATE TABLE IF NOT EXISTS member_profiles(server_id TEXT,user_id TEXT,name TEXT,color TEXT,avatar TEXT DEFAULT "",bio TEXT DEFAULT "",PRIMARY KEY(server_id,user_id))');this.c.storage.sql.exec('CREATE TABLE IF NOT EXISTS member_presence(server_id TEXT,user_id TEXT,updated INTEGER,PRIMARY KEY(server_id,user_id))');const match=u.pathname.match(/^\/api\/servers\/([\w-]+)\/members$/);if(match){const serverId=match[1];if(!this.member(serverId,user))return j({error:'Sem acesso'},403);this.ensureRoleSystem(serverId);if(req.method==='POST'){const body=await req.json(),now=Date.now(),name=String(body.name||user.name).slice(0,24),color=/^#[0-9a-f]{6}$/i.test(body.color)?body.color:user.color||'#5865f2';if(body.profile){const avatar=body.avatar===''?'':safeAvatar(body.avatar),bio=String(body.bio||'').trim().slice(0,160);if(body.avatar&&!avatar)return j({error:'Imagem de perfil inválida.'},400);this.c.storage.sql.exec('INSERT OR REPLACE INTO member_profiles(server_id,user_id,name,color,avatar,bio) VALUES(?,?,?,?,?,?)',serverId,user.id,name,color,avatar,bio)}else this.c.storage.sql.exec('INSERT OR IGNORE INTO member_profiles(server_id,user_id,name,color,avatar,bio) VALUES(?,?,?,?,"","")',serverId,user.id,name,color);this.c.storage.sql.exec('INSERT OR REPLACE INTO member_presence(server_id,user_id,updated) VALUES(?,?,?)',serverId,user.id,now);return j({ok:true,updated:now})}if(req.method==='GET'){const activeSince=Date.now()-65000;return j({members:[...this.c.storage.sql.exec('SELECT members.user_id,COALESCE(server_roles.name,members.role) AS role,server_roles.id AS role_id,server_roles.color AS role_color,COALESCE(server_roles.position,100) AS role_position,COALESCE(member_profiles.name,"Membro") AS name,COALESCE(member_profiles.color,"#5865f2") AS color,COALESCE(member_profiles.avatar,"") AS avatar,COALESCE(member_profiles.bio,"") AS bio,COALESCE(member_presence.updated,0) AS last_seen,CASE WHEN COALESCE(member_presence.updated,0)>=? THEN 1 ELSE 0 END AS online FROM members LEFT JOIN member_profiles ON members.server_id=member_profiles.server_id AND members.user_id=member_profiles.user_id LEFT JOIN member_presence ON members.server_id=member_presence.server_id AND members.user_id=member_presence.user_id LEFT JOIN member_roles ON members.server_id=member_roles.server_id AND members.user_id=member_roles.user_id LEFT JOIN server_roles ON server_roles.id=member_roles.role_id WHERE members.server_id=? ORDER BY COALESCE(server_roles.position,100),name',activeSince,serverId)]})}}return memberFetch.call(this,req)};
