...
Servers.prototype.categoryFor=function(serverId,id,type){if(!id)return '';return one(this.c.storage.sql.exec('SELECT id FROM channel_categories WHERE id=? AND server_id=? AND type=?',id,serverId,type))?.id||''};
Servers.prototype.fetch=async function(request){
  ...
  if(url.pathname===categoriesPath){
    if(request.method==='GET')return j({categories:[...this.c.storage.sql.exec('SELECT * FROM channel_categories WHERE server_id=? ORDER BY type,position,created',serverId)]});
    if(request.method==='POST'){
      if(!canManage)return j({error:'Sem permissão para criar categorias.'},403);const body=await request.json().catch(()=>({})),name=String(body.name||'').trim().replace(/\s+/g,' ').slice(0,32),type=body.type==='voice'?'voice':'text',color=/^#[0-9a-f]{6}$/i.test(body.color)?body.color:'#949cf7';
      if(name.length<2)return j({error:'O nome da categoria precisa ter pelo menos 2 caracteres.'},400);const position=Number(one(this.c.storage.sql.exec('SELECT COALESCE(MAX(position),-1)+1 AS next FROM channel_categories WHERE server_id=? AND type=?',serverId,type))?.next||0),category={id:crypto.randomUUID(),server_id:serverId,name,type,color,position,created:Date.now()};
      this.c.storage.sql.exec('INSERT INTO channel_categories VALUES(?,?,?,?,?,?,?)',category.id,serverId,name,type,color,position,category.created);this.audit(serverId,user,'CATEGORY_CREATE',category.id,name,`Categoria de ${type==='voice'?'voz':'texto'}`);return j({category},201);
    }
  }
  if(categoryItem){
  ...
  if(request.method==='GET'&&url.pathname===`/api/servers/${serverId}`){const data=await response.json();data.channelDetails=[...this.c.storage.sql.exec('SELECT name,category_id,position,topic FROM channels WHERE server_id=? ORDER BY position,rowid',serverId)].filter(channel=>this.canInChannel(serverId,user,'VIEW_CHANNELS','text',channel.name));data.categories=[...this.c.storage.sql.exec('SELECT * FROM channel_categories WHERE server_id=? ORDER BY type,position,created',serverId)];return j(data)}
  if(request.method==='GET'&&voiceCollection){return j({channels:[...this.c.storage.sql.exec('SELECT id,name,created,category_id,position FROM voice_channels WHERE server_id=? ORDER BY position,created,name',serverId)].filter(channel=>this.canInChannel(serverId,user,'CONNECT_VOICE','voice',channel.id)),categories:[...this.c.storage.sql.exec('SELECT * FROM channel_categories WHERE server_id=? AND type="voice" ORDER BY position,created',serverId)]})}
  if(request.method==='GET'&&url.pathname===`/api/servers/${serverId}/manage`){const data=await response.json();data.categories=[...this.c.storage.sql.exec('SELECT * FROM channel_categories WHERE server_id=? ORDER BY type,position,created',serverId)];data.textChannels=[...this.c.storage.sql.exec('SELECT name,category_id,position,topic FROM channels WHERE server_id=? ORDER BY position,rowid',serverId)];data.voiceChannels=[...this.c.storage.sql.exec('SELECT id,name,created,category_id,position FROM voice_channels WHERE server_id=? ORDER BY position,created,name',serverId)];return j(data)}
  return response;
};
