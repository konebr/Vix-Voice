...
Servers.prototype.ensureRoleSystem=function(serverId){
  ...
  const defaults=[['owner','Dono','#f4c45c',0,ALL_PERMISSIONS],['admin','Administrador','#a9a7ff',10,ALL_PERMISSIONS],['member','Membro','#747d90',100,PERMISSIONS.VIEW_CHANNELS|PERMISSIONS.SEND_MESSAGES|PERMISSIONS.CONNECT_VOICE|PERMISSIONS.CREATE_INVITES]];
...
Servers.prototype.fetch=async function(req){
  ...
  if(roleItem){const role=one(this.c.storage.sql.exec('SELECT * FROM server_roles WHERE id=? AND server_id=?',roleItem[1],serverId));if(!role)return j({error:'Cargo não encontrado.'},404);if(!can('MANAGE_ROLES'))return j({error:'Sem permissão para gerenciar cargos.'},403);if(!isOwner&&Number(role.position)<=Number(currentRole?.position??999))return j({error:'Você só pode editar cargos abaixo do seu.'},403);
    if(req.method==='PATCH'){if(role.position===0)return j({error:'O cargo Dono não pode ser alterado.'},409);const body=await req.json(),name=role.builtin?role.name:String(body.name||role.name).trim().slice(0,32),color=/^#[0-9a-f]{6}$/i.test(body.color)?body.color:role.color,permissions=permissionMask(body.permissions);if(name.length<2)return j({error:'Nome de cargo inválido.'},400);this.c.storage.sql.exec('UPDATE server_roles SET name=?,color=?,permissions=? WHERE id=?',name,color,permissions,role.id);this.audit(serverId,user,'ROLE_UPDATE',role.id,name,'Permissões atualizadas');return j({role:{...role,name,color,permissions}})}
