...
const publicUserId=id=>`VIX-${String(id||'').replace(/[^a-z0-9]/gi,'').slice(0,12).toUpperCase()}`;
...
export class Users{constructor(c,env){this.c=c;this.env=env;c.storage.sql.exec('CREATE TABLE IF NOT EXISTS users(id TEXT PRIMARY KEY,email TEXT UNIQUE,display_name TEXT,avatar_color TEXT,password_hash TEXT,created_at INTEGER,avatar TEXT DEFAULT "",bio TEXT DEFAULT "")');addColumn(c.storage.sql,'users','avatar TEXT DEFAULT ""');addColumn(c.storage.sql,'users','bio TEXT DEFAULT ""');addColumn(c.storage.sql,'users','banner TEXT DEFAULT ""');addColumn(c.storage.sql,'users','custom_status TEXT DEFAULT ""');addColumn(c.storage.sql,'users','pronouns TEXT DEFAULT ""');addColumn(c.storage.sql,'users','public_id TEXT DEFAULT ""');for(const user of [...c.storage.sql.exec('SELECT id FROM users WHERE public_id IS NULL OR public_id=""')])c.storage.sql.exec('UPDATE users SET public_id=? WHERE id=?',publicUserId(user.id),user.id);c.storage.sql.exec('CREATE UNIQUE INDEX IF NOT EXISTS users_public_id_unique ON users(public_id) WHERE public_id<>""');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS sessions(token TEXT PRIMARY KEY,user_id TEXT,expires_at INTEGER)')}async fetch(r){const u=new URL(r.url),token=r.headers.get('Authorization')?.replace('Bearer ','')||u.searchParams.get('token')||'';if(r.method==='POST'&&u.pathname==='/api/auth/register'){const x=await r.json(),email=String(x.email||'').trim().toLowerCase(),name=String(x.name||'').trim().slice(0,24),pass=String(x.password||'');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||name.length<2||pass.length<8)return j({error:'Informe nome, e-mail válido e senha com 8 caracteres.'},400);if(one(this.c.storage.sql.exec('SELECT id FROM users WHERE email=?',email)))return j({error:'Este e-mail já possui uma conta.'},409);const salt=crypto.getRandomValues(new Uint8Array(16)),id=crypto.randomUUID(),public_id=publicUserId(id),color=['#5865f2','#eb459e','#23a559','#f0b232'][Math.floor(Math.random()*4)];this.c.storage.sql.exec('INSERT INTO users(id,email,display_name,avatar_color,password_hash,created_at,public_id,avatar,bio) VALUES(?,?,?,?,?,?,?,"","")',id,email,name,color,`${b64(salt)}.${b64(await hash(pass,salt))}`,Date.now(),public_id);return this.session({id,email,name,color,public_id})}if(r.method==='POST'&&u.pathname==='/api/auth/login'){const x=await r.json(),z=one(this.c.storage.sql.exec('SELECT * FROM users WHERE email=?',String(x.email||'').trim().toLowerCase()));if(!z)return j({error:'E-mail ou senha incorretos.'},401);const[a,h]=z.password_hash.split('.').map(unb64),d=await hash(String(x.password||''),a);if(d.some((v,i)=>v!==h[i]))return j({error:'E-mail ou senha incorretos.'},401);return this.session(z)}if(r.method==='POST'&&u.pathname==='/api/auth/logout'){if(token)this.c.storage.sql.exec('DELETE FROM sessions WHERE token=?',token);return j({ok:true})}if(u.pathname==='/api/profile'){const profile=one(this.c.storage.sql.exec('SELECT users.id,users.public_id,users.display_name AS name,users.avatar_color AS color,COALESCE(users.avatar,"") AS avatar,COALESCE(users.bio,"") AS bio,COALESCE(users.banner,"") AS banner,COALESCE(users.custom_status,"") AS custom_status,COALESCE(users.pronouns,"") AS pronouns FROM sessions JOIN users ON users.id=sessions.user_id WHERE token=? AND expires_at>?',token,Date.now()));if(!profile)return j({error:'Não autenticado'},401);if(r.method==='GET')return j({profile});if(r.method==='PATCH'){const x=await r.json(),name=String(x.name||profile.name).trim().slice(0,24),color=/^#[0-9a-f]{6}$/i.test(x.color)?x.color:profile.color,bio=String(x.bio||'').trim().slice(0,190),custom_status=String(x.custom_status||'').trim().slice(0,60),pronouns=String(x.pronouns||'').trim().slice(0,40),avatar=x.avatar===''?'':safeAvatar(x.avatar||profile.avatar),banner=x.banner===''?'':safeBanner(x.banner||profile.banner);if(name.length<2)return j({error:'O nome precisa ter pelo menos 2 caracteres.'},400);if(x.avatar&&!avatar)return j({error:'A imagem do perfil é inválida ou muito grande.'},400);if(x.banner&&!banner)return j({error:'O banner é inválido ou muito grande.'},400);this.c.storage.sql.exec('UPDATE users SET display_name=?,avatar_color=?,avatar=?,bio=?,banner=?,custom_status=?,pronouns=? WHERE id=?',name,color,avatar,bio,banner,custom_status,pronouns,profile.id);return j({profile:{...profile,name,color,avatar,bio,banner,custom_status,pronouns}})}return j({error:'Método inválido'},405)}if(u.pathname==='/session'){const z=one(this.c.storage.sql.exec('SELECT users.id,users.email,users.public_id,users.display_name AS name,users.avatar_color AS color FROM sessions JOIN users ON users.id=sessions.user_id WHERE token=? AND expires_at>?',u.searchParams.get('token'),Date.now()));return z?j({user:z}):j({error:'Sessão expirada'},401)}return j({error:'Rota inexistente'},404)}session(u){const t=crypto.randomUUID()+crypto.randomUUID().replaceAll('-','');this.c.storage.sql.exec('INSERT INTO sessions VALUES(?,?,?)',t,u.id,Date.now()+2592000000);return j({token:t,user:{id:u.id,email:u.email,public_id:u.public_id||publicUserId(u.id),name:u.name||u.display_name,color:u.color||u.avatar_color,avatar:u.avatar||'',bio:u.bio||'',banner:u.banner||'',custom_status:u.custom_status||'',pronouns:u.pronouns||''}})}}

const CARD_THEMES=new Set(['midnight','aurora','neon','sunset','ocean','royal','forest','rose']),CARD_EFFECTS=new Set(['none','glow','crystal','stars']),CARD_BORDERS=new Set(['subtle','solid','gradient','neon','gold','prismatic']),AVATAR_FRAMES=new Set(['classic','ring','double','neon','crown']);
const baseUserFetch=Users.prototype.fetch;
Users.prototype.fetch=async function(request){const url=new URL(request.url);if(url.pathname!=='/api/profile/card')return baseUserFetch.call(this,request);this.c.storage.sql.exec('CREATE TABLE IF NOT EXISTS profile_cards(user_id TEXT PRIMARY KEY,theme TEXT,effect TEXT,badge TEXT)');addColumn(this.c.storage.sql,'profile_cards','border_style TEXT DEFAULT "subtle"');addColumn(this.c.storage.sql,'profile_cards','avatar_frame TEXT DEFAULT "classic"');const token=request.headers.get('Authorization')?.replace('Bearer ','')||url.searchParams.get('token')||'',user=one(this.c.storage.sql.exec('SELECT users.id FROM sessions JOIN users ON users.id=sessions.user_id WHERE token=? AND expires_at>?',token,Date.now()));if(!user)return j({error:'Não autenticado'},401);const saved=one(this.c.storage.sql.exec('SELECT theme,effect,badge,COALESCE(border_style,"subtle") AS border_style,COALESCE(avatar_frame,"classic") AS avatar_frame FROM profile_cards WHERE user_id=?',user.id))||{theme:'midnight',effect:'none',badge:'',border_style:'subtle',avatar_frame:'classic'};if(request.method==='GET')return j({card:saved});if(request.method!=='PATCH')return j({error:'Método inválido'},405);const body=await request.json(),theme=CARD_THEMES.has(body.theme)?body.theme:'midnight',effect=CARD_EFFECTS.has(body.effect)?body.effect:'none',badge=String(body.badge||'').trim().slice(0,24),border_style=CARD_BORDERS.has(body.border_style)?body.border_style:(CARD_BORDERS.has(saved.border_style)?saved.border_style:'subtle'),avatar_frame=AVATAR_FRAMES.has(body.avatar_frame)?body.avatar_frame:(AVATAR_FRAMES.has(saved.avatar_frame)?saved.avatar_frame:'classic');this.c.storage.sql.exec('INSERT OR REPLACE INTO profile_cards(user_id,theme,effect,badge,border_style,avatar_frame) VALUES(?,?,?,?,?,?)',user.id,theme,effect,badge,border_style,avatar_frame);return j({card:{theme,effect,badge,border_style,avatar_frame}})};
...
export class Servers{constructor(c,env){this.c=c;this.env=env;c.storage.sql.exec('CREATE TABLE IF NOT EXISTS servers(id TEXT PRIMARY KEY,name TEXT,icon TEXT,owner TEXT,invite TEXT UNIQUE,created INTEGER)');addColumn(c.storage.sql,'servers','image TEXT DEFAULT ""');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS server_images(server_id TEXT PRIMARY KEY,image TEXT NOT NULL,updated INTEGER NOT NULL)');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS members(server_id TEXT,user_id TEXT,role TEXT,PRIMARY KEY(server_id,user_id))');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS channels(server_id TEXT,name TEXT,PRIMARY KEY(server_id,name))');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS messages(id TEXT PRIMARY KEY,server_id TEXT,channel TEXT,author TEXT,author_id TEXT,text TEXT,created INTEGER,edited INTEGER)');addColumn(c.storage.sql,'messages','reply_to TEXT DEFAULT ""');addColumn(c.storage.sql,'messages','updated INTEGER DEFAULT 0');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS reactions(message_id TEXT,emoji TEXT,user_id TEXT,PRIMARY KEY(message_id,emoji,user_id))');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS message_deletions(id TEXT PRIMARY KEY,server_id TEXT,deleted INTEGER)');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS message_attachments(id TEXT PRIMARY KEY,message_id TEXT UNIQUE,name TEXT,type TEXT,size INTEGER,data TEXT)');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS message_pins(server_id TEXT,message_id TEXT PRIMARY KEY,pinned_by TEXT,pinned_at INTEGER)');c.storage.sql.exec('CREATE TABLE IF NOT EXISTS member_profiles(server_id TEXT,user_id TEXT,name TEXT,color TEXT,avatar TEXT DEFAULT "",bio TEXT DEFAULT "",PRIMARY KEY(server_id,user_id))');addColumn(c.storage.sql,'member_profiles','avatar TEXT DEFAULT ""');addColumn(c.storage.sql,'member_profiles','bio TEXT DEFAULT ""');addColumn(c.storage.sql,'member_profiles','banner TEXT DEFAULT ""');addColumn(c.storage.sql,'member_profiles','custom_status TEXT DEFAULT ""');addColumn(c.storage.sql,'member_profiles','pronouns TEXT DEFAULT ""');addColumn(c.storage.sql,'member_profiles','card_theme TEXT DEFAULT "midnight"');addColumn(c.storage.sql,'member_profiles','card_effect TEXT DEFAULT "none"');addColumn(c.storage.sql,'member_profiles','profile_badge TEXT DEFAULT ""');addColumn(c.storage.sql,'member_profiles','card_border TEXT DEFAULT "subtle"');addColumn(c.storage.sql,'member_profiles','avatar_frame TEXT DEFAULT "classic"');}
...
Servers.prototype.fetch=async function(req){
  ...
  const seedVoiceChannels=id=>{if(one(this.c.storage.sql.exec('SELECT server_id FROM channel_layout_state WHERE server_id=?',id)))return;if(!one(this.c.storage.sql.exec('SELECT id FROM voice_channels WHERE server_id=? LIMIT 1',id)))this.c.storage.sql.exec('INSERT INTO voice_channels(id,server_id,name,created) VALUES(?,?,?,?)',crypto.randomUUID(),id,'Geral',Date.now())};
  const server=serverId&&this.serverWithImage(serverId);
  ...
  if(server&&u.pathname===`/api/servers/${serverId}`&&req.method==='PATCH'){
    ...
    const body=managedBody,name=String(body.name||'').trim().slice(0,32),icon=[...String(body.icon||'').trim()].slice(0,2).join(''),image=body.image===''?'':safeAvatar(body.image||server.image);
    if(name.length<2)return j({error:'O nome precisa ter pelo menos 2 caracteres.'},400);
    if(!icon&&!image)return j({error:'Escolha um ícone ou uma foto.'},400);
    if(body.image&&!image)return j({error:'A foto do servidor é inválida ou muito grande.'},400);
    const fallbackIcon=icon||name.slice(0,1).toUpperCase();
    this.c.storage.transactionSync(()=>{this.c.storage.sql.exec('UPDATE servers SET name=?,icon=?,image=? WHERE id=?',name,fallbackIcon,image,serverId);this.c.storage.sql.exec('INSERT OR REPLACE INTO server_images VALUES(?,?,?)',serverId,image,Date.now())});
    const saved=this.serverWithImage(serverId);if(saved.image!==image)return j({error:'Não foi possível confirmar a foto salva.'},500);
    return j({server:saved});
  }
  ...
  if(voiceCollection){
    ...
    if(req.method==='POST'){
      ...
      const channel={id:crypto.randomUUID(),name,created:Date.now()};this.c.storage.sql.exec('INSERT INTO voice_channels(id,server_id,name,created) VALUES(?,?,?,?)',channel.id,serverId,channel.name,channel.created);return j({channel},201);
    }
  }
...
Servers.prototype.fetch=async function(req){
  ...
  const server=this.serverWithImage(serverId),isOwner=server?.owner===user.id,currentRole=this.roleFor(serverId,user.id),can=permission=>this.can(serverId,user,permission);
...
Users.prototype.privateUser=function(req){
  ...
  return one(this.c.storage.sql.exec('SELECT users.id,users.email,users.public_id,users.display_name AS name,users.avatar_color AS color FROM sessions JOIN users ON users.id=sessions.user_id WHERE token=? AND expires_at>?',token,Date.now()));
};
...
Users.prototype.fetch=async function(req){
  ...
  if(u.pathname==='/api/private/home'&&req.method==='GET'){
    ...
    for(const link of links){
      const id=link.user_a===user.id?link.user_b:link.user_a,person=one(this.c.storage.sql.exec('SELECT id,public_id,display_name AS name,avatar_color AS color,COALESCE(avatar,"") AS avatar,COALESCE(banner,"") AS banner,COALESCE(custom_status,"") AS custom_status,COALESCE(pronouns,"") AS pronouns,COALESCE(bio,"") AS bio FROM users WHERE id=?',id));if(!person)continue;
    ...
    const incoming=[...this.c.storage.sql.exec('SELECT friend_requests.id,friend_requests.created,users.id AS user_id,users.public_id,users.display_name AS name,users.avatar_color AS color,COALESCE(users.avatar,"") AS avatar FROM friend_requests JOIN users ON users.id=friend_requests.sender_id WHERE receiver_id=? AND status=? ORDER BY friend_requests.created DESC',user.id,'pending')];
    const outgoing=[...this.c.storage.sql.exec('SELECT friend_requests.id,friend_requests.created,users.id AS user_id,users.public_id,users.display_name AS name,users.avatar_color AS color,COALESCE(users.avatar,"") AS avatar FROM friend_requests JOIN users ON users.id=friend_requests.receiver_id WHERE sender_id=? AND status=? ORDER BY friend_requests.created DESC',user.id,'pending')];
  ...
  const privateCallToken=u.pathname.match(/^\/api\/private\/call-token\/([\w-]+)\/([\w-]+)$/);
  if(privateCallToken&&req.method==='POST'){
    const friendId=privateCallToken[1],callId=privateCallToken[2];
    if(!this.privateFriends(user.id,friendId))return j({error:'A chamada exige uma amizade aceita.'},403);
    ...
    if(apiKey.length<8||apiSecret.length<32||!/^wss:\/\//i.test(endpoint))return j({error:'Servidor de voz da VPS ainda não configurado.'},503);
    const body=await req.json().catch(()=>({})),connectionId=String(body.connection_id||crypto.randomUUID()).replace(/[^a-zA-Z0-9-]/g,'').slice(0,64)||crypto.randomUUID(),members=[user.id,friendId].sort(),room=`private:${members[0]}:${members[1]}:${callId}`,now=Math.floor(Date.now()/1000);
    const token=await signSfuToken(apiKey,apiSecret,{iss:apiKey,sub:`${user.id}:${connectionId}`,name:user.name,nbf:now-5,exp:now+7200,metadata:JSON.stringify({userId:user.id,color:user.color||'#5865f2',privateCall:true}),video:{roomJoin:true,room,canPublish:true,canSubscribe:true,canPublishData:false,canPublishSources:['microphone']}});
    return j({url:endpoint,token,room,route:'SFU da VPS'});
  }
...
Servers.prototype.fetch=async function(request){
  ...
  const server=this.serverWithImage(serverId),isOwner=server?.owner===user.id,member=this.member(serverId,user),can=permission=>this.can(serverId,user,permission);
...
// Organização livre de categorias e canais. Mantém servidores vazios quando o criador remove todas as salas.
const channelOrganizationFetch=Servers.prototype.fetch;
Servers.prototype.ensureChannelOrganization=function(serverId){
  this.c.storage.sql.exec('CREATE TABLE IF NOT EXISTS channel_categories(id TEXT PRIMARY KEY,server_id TEXT,name TEXT,type TEXT,color TEXT,position INTEGER,created INTEGER)');
  this.c.storage.sql.exec('CREATE TABLE IF NOT EXISTS channel_layout_state(server_id TEXT PRIMARY KEY,voice_initialized INTEGER,text_initialized INTEGER)');
  this.c.storage.sql.exec('CREATE TABLE IF NOT EXISTS channel_schema_migrations(name TEXT PRIMARY KEY,applied INTEGER)');
  this.c.storage.sql.exec('CREATE TABLE IF NOT EXISTS voice_channels(id TEXT PRIMARY KEY,server_id TEXT,name TEXT,created INTEGER,UNIQUE(server_id,name))');
  addColumn(this.c.storage.sql,'channels','category_id TEXT DEFAULT ""');addColumn(this.c.storage.sql,'channels','position INTEGER DEFAULT 0');addColumn(this.c.storage.sql,'channels','topic TEXT DEFAULT ""');
  addColumn(this.c.storage.sql,'voice_channels','category_id TEXT DEFAULT ""');addColumn(this.c.storage.sql,'voice_channels','position INTEGER DEFAULT 0');
  if(!one(this.c.storage.sql.exec('SELECT name FROM channel_schema_migrations WHERE name="voice_names_by_category"'))){
    this.c.storage.transactionSync(()=>{this.c.storage.sql.exec('ALTER TABLE voice_channels RENAME TO voice_channels_global_names');this.c.storage.sql.exec('CREATE TABLE voice_channels(id TEXT PRIMARY KEY,server_id TEXT,name TEXT,created INTEGER,category_id TEXT DEFAULT "",position INTEGER DEFAULT 0)');this.c.storage.sql.exec('INSERT INTO voice_channels(id,server_id,name,created,category_id,position) SELECT id,server_id,name,created,COALESCE(category_id,""),COALESCE(position,0) FROM voice_channels_global_names');this.c.storage.sql.exec('DROP TABLE voice_channels_global_names');this.c.storage.sql.exec('INSERT INTO channel_schema_migrations VALUES("voice_names_by_category",?)',Date.now())});
  }
  if(!one(this.c.storage.sql.exec('SELECT server_id FROM channel_layout_state WHERE server_id=?',serverId))){
    const hasVoice=!!one(this.c.storage.sql.exec('SELECT id FROM voice_channels WHERE server_id=? LIMIT 1',serverId));
    if(!hasVoice)this.c.storage.sql.exec('INSERT INTO voice_channels(id,server_id,name,created,category_id,position) VALUES(?,?,?,?,"",0)',crypto.randomUUID(),serverId,'Geral',Date.now());
    this.c.storage.sql.exec('INSERT INTO channel_layout_state VALUES(?,1,1)',serverId);
  }
};
Servers.prototype.categoryFor=function(serverId,id){if(!id)return '';return one(this.c.storage.sql.exec('SELECT id FROM channel_categories WHERE id=? AND server_id=?',id,serverId))?.id||''};
Servers.prototype.fetch=async function(request){
  const url=new URL(request.url),serverId=url.pathname.match(/^\/api\/servers\/([\w-]+)(?:\/|$)/)?.[1];
  if(!serverId)return channelOrganizationFetch.call(this,request);
  const user=await this.user(request);if(!user)return j({error:'Não autenticado'},401);
  this.ensureChannelOrganization(serverId);
  const member=this.member(serverId,user),canManage=this.can(serverId,user,'MANAGE_CHANNELS');
  if(!member)return channelOrganizationFetch.call(this,request);
  const categoriesPath=`/api/servers/${serverId}/categories`,categoryItem=url.pathname.match(new RegExp(`^/api/servers/${serverId}/categories/([\\w-]+)$`));
  if(url.pathname===categoriesPath){
    if(request.method==='GET')return j({categories:[...this.c.storage.sql.exec('SELECT * FROM channel_categories WHERE server_id=? ORDER BY position,created',serverId)]});
    if(request.method==='POST'){
      if(!canManage)return j({error:'Sem permissão para criar categorias.'},403);const body=await request.json().catch(()=>({})),name=String(body.name||'').trim().replace(/\s+/g,' ').slice(0,32),type='mixed',color=/^#[0-9a-f]{6}$/i.test(body.color)?body.color:'#949cf7';
      if(name.length<2)return j({error:'O nome da categoria precisa ter pelo menos 2 caracteres.'},400);const position=Number(one(this.c.storage.sql.exec('SELECT COALESCE(MAX(position),-1)+1 AS next FROM channel_categories WHERE server_id=?',serverId))?.next||0),category={id:crypto.randomUUID(),server_id:serverId,name,type,color,position,created:Date.now()};
      this.c.storage.sql.exec('INSERT INTO channel_categories VALUES(?,?,?,?,?,?,?)',category.id,serverId,name,type,color,position,category.created);this.audit(serverId,user,'CATEGORY_CREATE',category.id,name,'Categoria de canais');return j({category},201);
    }
  }
  if(categoryItem){
    if(!canManage)return j({error:'Sem permissão para editar categorias.'},403);const category=one(this.c.storage.sql.exec('SELECT * FROM channel_categories WHERE id=? AND server_id=?',categoryItem[1],serverId));if(!category)return j({error:'Categoria não encontrada.'},404);
    if(request.method==='PATCH'){const body=await request.json().catch(()=>({})),name=String(body.name||category.name).trim().replace(/\s+/g,' ').slice(0,32),color=/^#[0-9a-f]{6}$/i.test(body.color)?body.color:category.color,position=Math.max(0,Math.min(999,Number(body.position) || 0));if(name.length<2)return j({error:'Nome de categoria inválido.'},400);this.c.storage.sql.exec('UPDATE channel_categories SET name=?,color=?,position=? WHERE id=?',name,color,position,category.id);this.audit(serverId,user,'CATEGORY_UPDATE',category.id,name,'Categoria personalizada');return j({category:{...category,name,color,position}})}
    if(request.method==='DELETE'){this.c.storage.transactionSync(()=>{this.c.storage.sql.exec('UPDATE channels SET category_id="" WHERE server_id=? AND category_id=?',serverId,category.id);this.c.storage.sql.exec('UPDATE voice_channels SET category_id="" WHERE server_id=? AND category_id=?',serverId,category.id);this.c.storage.sql.exec('DELETE FROM channel_categories WHERE id=?',category.id)});this.audit(serverId,user,'CATEGORY_DELETE',category.id,category.name,'Canais movidos para sem categoria');return j({ok:true})}
  }
  const textCollection=url.pathname===`/api/servers/${serverId}/channels`,textItem=url.pathname.match(new RegExp(`^/api/servers/${serverId}/channels/([\\w-]+)$`)),voiceCollection=url.pathname===`/api/servers/${serverId}/voice-channels`,voiceItem=url.pathname.match(new RegExp(`^/api/servers/${serverId}/voice-channels/([\\w-]+)$`));
  if(textItem&&request.method==='DELETE'){
    if(!canManage)return j({error:'Sem permissão para excluir canais.'},403);const name=textItem[1],saved=one(this.c.storage.sql.exec('SELECT name FROM channels WHERE server_id=? AND name=?',serverId,name));if(!saved)return j({error:'Canal não encontrado.'},404);this.c.storage.transactionSync(()=>{this.c.storage.sql.exec('DELETE FROM reactions WHERE message_id IN (SELECT id FROM messages WHERE server_id=? AND channel=?)',serverId,name);this.c.storage.sql.exec('DELETE FROM messages WHERE server_id=? AND channel=?',serverId,name);this.c.storage.sql.exec('DELETE FROM channels WHERE server_id=? AND name=?',serverId,name);this.c.storage.sql.exec('DELETE FROM channel_permissions WHERE server_id=? AND channel_type="text" AND channel_id=?',serverId,name)});this.audit(serverId,user,'CHANNEL_DELETE',name,name,'Canal de texto excluído');return j({ok:true});
  }
  if(voiceItem&&request.method==='DELETE'){
    if(!canManage)return j({error:'Sem permissão para excluir canais.'},403);const id=voiceItem[1],saved=one(this.c.storage.sql.exec('SELECT id,name FROM voice_channels WHERE server_id=? AND id=?',serverId,id));if(!saved)return j({error:'Canal de voz não encontrado.'},404);this.disconnectVoiceMember&&[...this.c.storage.sql.exec('SELECT user_id FROM voice_presence WHERE server_id=? AND channel=?',serverId,id)].forEach(item=>this.disconnectVoiceMember(serverId,item.user_id,'Canal removido'));this.c.storage.sql.exec('DELETE FROM voice_channels WHERE id=?',id);this.c.storage.sql.exec('DELETE FROM voice_presence WHERE server_id=? AND channel=?',serverId,id);this.c.storage.sql.exec('DELETE FROM channel_permissions WHERE server_id=? AND channel_type="voice" AND channel_id=?',serverId,id);this.audit(serverId,user,'CHANNEL_DELETE',id,saved.name,'Canal de voz excluído');return j({ok:true});
  }
  const bodyNeeded=(request.method==='POST'&&(textCollection||voiceCollection))||(request.method==='PATCH'&&(textItem||voiceItem)),body=bodyNeeded?await request.clone().json().catch(()=>({})):{};
  if(request.method==='POST'&&voiceCollection){
    if(!canManage)return j({error:'Sem permissão para gerenciar canais.'},403);const name=String(body.name||'').trim().replace(/\s+/g,' ').slice(0,32),categoryId=this.categoryFor(serverId,body.category_id),position=Math.max(0,Number(body.position)||0);
    if(name.length<2)return j({error:'Nome de canal inválido.'},400);if(one(this.c.storage.sql.exec('SELECT id FROM voice_channels WHERE server_id=? AND COALESCE(category_id,"")=? AND lower(name)=lower(?)',serverId,categoryId,name)))return j({error:'Já existe uma sala de voz com esse nome nesta categoria.'},409);
    const channel={id:crypto.randomUUID(),name,created:Date.now(),category_id:categoryId,position};this.c.storage.sql.exec('INSERT INTO voice_channels(id,server_id,name,created,category_id,position) VALUES(?,?,?,?,?,?)',channel.id,serverId,name,channel.created,categoryId,position);this.audit(serverId,user,'CHANNEL_CREATE',channel.id,name,'Canal de voz criado');return j({channel},201);
  }
  if(request.method==='PATCH'&&voiceItem){
    if(!canManage)return j({error:'Sem permissão para gerenciar canais.'},403);const current=one(this.c.storage.sql.exec('SELECT * FROM voice_channels WHERE server_id=? AND id=?',serverId,voiceItem[1]));if(!current)return j({error:'Canal de voz não encontrado.'},404);const name=String(body.name||current.name).trim().replace(/\s+/g,' ').slice(0,32),categoryId=Object.hasOwn(body,'category_id')?this.categoryFor(serverId,body.category_id):current.category_id||'',position=Object.hasOwn(body,'position')?Math.max(0,Number(body.position)||0):Number(current.position)||0;
    if(name.length<2)return j({error:'Nome de canal inválido.'},400);if(one(this.c.storage.sql.exec('SELECT id FROM voice_channels WHERE server_id=? AND COALESCE(category_id,"")=? AND lower(name)=lower(?) AND id<>?',serverId,categoryId,name,current.id)))return j({error:'Já existe uma sala de voz com esse nome nesta categoria.'},409);
    this.c.storage.sql.exec('UPDATE voice_channels SET name=?,category_id=?,position=? WHERE id=?',name,categoryId,position,current.id);this.audit(serverId,user,'CHANNEL_UPDATE',current.id,name,'Canal de voz personalizado');return j({channel:{...current,name,category_id:categoryId,position}});
  }
  const response=await channelOrganizationFetch.call(this,request);if(!response.ok)return response;
  if(request.method==='POST'&&textCollection){const data=await response.json(),categoryId=this.categoryFor(serverId,body.category_id,'text');this.c.storage.sql.exec('UPDATE channels SET category_id=?,position=?,topic=? WHERE server_id=? AND name=?',categoryId,Math.max(0,Number(body.position)||0),String(body.topic||'').trim().slice(0,160),serverId,data.name);return j({...data,channel:{name:data.name,category_id:categoryId,position:Math.max(0,Number(body.position)||0),topic:String(body.topic||'').trim().slice(0,160)}},201)}
  if(request.method==='PATCH'&&textItem){const data=await response.json(),current=one(this.c.storage.sql.exec('SELECT category_id,position,topic FROM channels WHERE server_id=? AND name=?',serverId,data.name))||{},categoryId=Object.hasOwn(body,'category_id')?this.categoryFor(serverId,body.category_id,'text'):current.category_id||'',topic=Object.hasOwn(body,'topic')?String(body.topic||'').trim().slice(0,160):current.topic||'',position=Object.hasOwn(body,'position')?Math.max(0,Number(body.position)||0):Number(current.position)||0;this.c.storage.sql.exec('UPDATE channels SET category_id=?,position=?,topic=? WHERE server_id=? AND name=?',categoryId,position,topic,serverId,data.name);return j({...data,channel:{name:data.name,category_id:categoryId,position,topic}})}
  if(request.method==='POST'&&voiceCollection){const data=await response.json(),categoryId=this.categoryFor(serverId,body.category_id,'voice'),position=Math.max(0,Number(body.position)||0);this.c.storage.sql.exec('UPDATE voice_channels SET category_id=?,position=? WHERE id=?',categoryId,position,data.channel.id);return j({channel:{...data.channel,category_id:categoryId,position}},201)}
  if(request.method==='PATCH'&&voiceItem){const data=await response.json(),current=one(this.c.storage.sql.exec('SELECT category_id,position FROM voice_channels WHERE id=?',data.channel.id))||{},categoryId=Object.hasOwn(body,'category_id')?this.categoryFor(serverId,body.category_id,'voice'):current.category_id||'',position=Object.hasOwn(body,'position')?Math.max(0,Number(body.position)||0):Number(current.position)||0;this.c.storage.sql.exec('UPDATE voice_channels SET category_id=?,position=? WHERE id=?',categoryId,position,data.channel.id);return j({channel:{...data.channel,category_id:categoryId,position}})}
  if(request.method==='GET'&&url.pathname===`/api/servers/${serverId}`){const data=await response.json();data.channelDetails=[...this.c.storage.sql.exec('SELECT name,category_id,position,topic FROM channels WHERE server_id=? ORDER BY position,rowid',serverId)].filter(channel=>this.canInChannel(serverId,user,'VIEW_CHANNELS','text',channel.name));data.categories=[...this.c.storage.sql.exec('SELECT * FROM channel_categories WHERE server_id=? ORDER BY position,created',serverId)];return j(data)}
  if(request.method==='GET'&&voiceCollection){return j({channels:[...this.c.storage.sql.exec('SELECT id,name,created,category_id,position FROM voice_channels WHERE server_id=? ORDER BY position,created,name',serverId)].filter(channel=>this.canInChannel(serverId,user,'CONNECT_VOICE','voice',channel.id)),categories:[...this.c.storage.sql.exec('SELECT * FROM channel_categories WHERE server_id=? ORDER BY position,created',serverId)]})}
