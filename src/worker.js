...
Servers.prototype.fetch=async function(req){
  ...
  if(server&&u.pathname===`/api/servers/${serverId}`&&req.method==='PATCH'){
    ...
    this.c.storage.sql.exec('UPDATE servers SET name=?,icon=?,image=? WHERE id=?',name,fallbackIcon,image,serverId);
    return j({server:{...server,name,icon:fallbackIcon,image}});
  }
