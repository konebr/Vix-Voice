(() => {
  ...
  function watch(id) {
    ...
    const media = streamMedia(item); video.muted = true; if (video.srcObject !== media) video.srcObject = media;
  ...
  for (const event of ['loadeddata','canplay','playing']) video.addEventListener(event, () => { if (selected && video.videoWidth) status.textContent = ''; });

  let localTrack = null;
  setInterval(() => { const track = screenStream?.getVideoTracks?.().find(item => item.readyState === 'live') || null; if (track === localTrack) return; localTrack = track; if (!track) return remove(state.identity?.id || 'local'); const local = { id: String(state.identity?.id || 'local'), name: `${state.identity?.name || 'Você'} (sua transmissão)`, stream: screenStream, local: true }; streams.set(local.id, local); render(); }, 400);
  render();
})();
