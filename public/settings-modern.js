(() => {
  ...
  function renderAppearance() { const contrast = document.body.classList.contains('vix-contrast'), current = document.documentElement.dataset.theme || 'dark'; $('settings-content').innerHTML = `<section class="settings-page"><div class="settings-page-intro"><span>INTERFACE</span><h1>Aparência</h1><p>Escolha o visual e ajuste a leitura neste dispositivo.</p></div><div class="settings-section"><h2>Tema</h2><div class="vix-theme-grid">${[['dark','Escuro','Equilibrado e confortável'],['midnight','Meia-noite','Profundo com detalhes neon'],['light','Claro','Limpo para ambientes iluminados']].map(([id,name,copy])=>`<button type="button" data-theme-choice="${id}" class="${current===id?'selected':''}"><span class="theme-preview ${id}"></span><strong>${name}</strong><small>${copy}</small></button>`).join('')}</div>${notificationRow('appearance-contrast','Contraste reforçado','Realça divisórias, textos e controles.',contrast)}</div></section>`; for(const button of document.querySelectorAll('[data-theme-choice]'))button.onclick=()=>{vixApplyTheme(button.dataset.themeChoice);renderAppearance()}; $('appearance-contrast').onchange = event => document.body.classList.toggle('vix-contrast', event.target.checked); }
