(() => {
  ...
  async function renderAccount() {
    ...
    $('settings-content').innerHTML = `<section class="settings-page settings-account-page"><div class="settings-page-intro"><span>CONTA VIX</span><h1>Minha conta</h1><p>Consulte seus dados de acesso e mantenha sua conta protegida.</p></div>
      ...
      <div class="settings-section"><div class="settings-section-title"><div><h2>Informações da conta</h2><p>Dados usados para entrar no Vix Voice.</p></div></div><div class="account-details"><label>Nome de exibição<strong>${escapeHtml(state.identity.name)}</strong></label><label>ID público<strong>${escapeHtml(state.identity.public_id||'Gerando…')}</strong></label><label>Endereço de e-mail<strong>${email}</strong></label></div></div>
  ...
  function updateInlinePreview() {
    ...
    preview.className = `inline-profile-preview card-theme-${$('inline-card-theme').value} card-effect-${$('inline-card-effect').value} card-border-${$('inline-card-border').value} avatar-frame-${$('inline-avatar-frame').value}`;
  ...
  async function renderProfile() {
    ...
    try { const [{ profile }, { card }] = await Promise.all([api('/api/profile'), api('/api/profile/card')]); state.identity = { ...state.identity, ...profile, card_theme: card.theme, card_effect: card.effect, profile_badge: card.badge, card_border:card.border_style, avatar_frame:card.avatar_frame }; } catch (error) { console.warn(error); }
    ...
    $('settings-content').innerHTML = `<section class="settings-page"><div class="settings-page-intro"><span>IDENTIDADE VIX</span><h1>Personalize seu perfil</h1><p>As alterações aparecem no seu cartão, nas conversas e na lista de membros.</p></div><div class="inline-profile-layout"><form id="inline-profile-form" class="settings-section inline-profile-form">
      ...
      <div class="inline-card-style"><h3>Estilo do cartão</h3><div class="settings-form-grid"><label>Tema<select id="inline-card-theme"><option value="midnight">Meia-noite</option><option value="aurora">Aurora</option><option value="neon">Neon</option><option value="sunset">Pôr do sol</option><option value="ocean">Oceano</option><option value="royal">Realeza</option><option value="forest">Floresta</option><option value="rose">Rosa noturno</option></select></label><label>Efeito<select id="inline-card-effect"><option value="none">Sem efeito</option><option value="glow">Brilho</option><option value="crystal">Cristais</option><option value="stars">Estrelas</option></select></label><label>Borda do cartão<select id="inline-card-border"><option value="subtle">Discreta</option><option value="solid">Cor de destaque</option><option value="gradient">Gradiente</option><option value="neon">Neon animado</option><option value="gold">Dourada</option><option value="prismatic">Prismática</option></select></label><label>Moldura do avatar<select id="inline-avatar-frame"><option value="classic">Clássica</option><option value="ring">Anel colorido</option><option value="double">Anel duplo</option><option value="neon">Neon</option><option value="crown">Coroa</option></select></label></div><label>Etiqueta do perfil<input id="inline-profile-badge" maxlength="24" value="${escapeHtml(state.identity.profile_badge || '')}" placeholder="Ex.: Fundador, Gamer, Artista"></label></div>
    ...
    $('inline-card-theme').value = state.identity.card_theme || 'midnight'; $('inline-card-effect').value = state.identity.card_effect || 'none'; $('inline-card-border').value = state.identity.card_border || 'subtle'; $('inline-avatar-frame').value = state.identity.avatar_frame || 'classic';
    ...
    for (const id of ['inline-card-theme','inline-card-effect','inline-card-border','inline-avatar-frame']) $(id).onchange = updateInlinePreview;
  ...
  function renderAppearance() {
    const contrast = document.body.classList.contains('vix-contrast'), current = document.documentElement.dataset.theme || 'dark', themes = globalThis.vixThemes || [];
    const custom = globalThis.vixCustomTheme?.() || { bg:'#151923', panel:'#242938', accent:'#6875ff' };
    const cards = themes.map(([id,name,copy,bg,panel,accent]) => `<button type="button" data-theme-choice="${id}" class="${current===id?'selected':''}" aria-pressed="${current===id}"><span class="theme-preview" style="--preview-bg:${id==='custom'?custom.bg:bg};--preview-panel:${id==='custom'?custom.panel:panel};--preview-accent:${id==='custom'?custom.accent:accent}"></span><strong>${name}${current===id?' <b>✓</b>':''}</strong><small>${copy}</small></button>`).join('');
    $('settings-content').innerHTML = `<section class="settings-page appearance-page"><div class="settings-page-intro"><span>PERSONALIZAÇÃO</span><h1>Temas do Vix Voice</h1><p>Pré-visualize uma combinação e deixe o programa com o seu estilo.</p></div><div class="settings-section theme-gallery"><div class="settings-section-title"><div><h2>Temas coloridos</h2><p>A mudança é aplicada imediatamente e salva neste dispositivo.</p></div><span class="theme-count">${themes.length} OPÇÕES</span></div><div class="vix-theme-grid">${cards}</div></div><form id="custom-theme-form" class="settings-section custom-theme-editor"><div class="settings-section-title"><div><h2>Crie seu próprio tema</h2><p>Combine as cores principais da interface.</p></div><button type="submit" class="settings-primary">Aplicar tema</button></div><div class="custom-theme-colors"><label>Fundo<input name="bg" type="color" value="${custom.bg}"></label><label>Painéis<input name="panel" type="color" value="${custom.panel}"></label><label>Destaque<input name="accent" type="color" value="${custom.accent}"></label><div class="custom-theme-live"><i></i><i></i><i></i><span>Prévia</span></div></div></form><div class="settings-section">${notificationRow('appearance-contrast','Contraste reforçado','Realça divisórias, textos e controles.',contrast)}</div></section>`;
    for (const button of document.querySelectorAll('[data-theme-choice]')) button.onclick = () => { vixApplyTheme(button.dataset.themeChoice); renderAppearance(); };
    const form = $('custom-theme-form'), preview = form.querySelector('.custom-theme-live');
    const updatePreview = () => { preview.style.setProperty('--custom-bg', form.elements.bg.value); preview.style.setProperty('--custom-panel', form.elements.panel.value); preview.style.setProperty('--custom-accent', form.elements.accent.value); };
    for (const input of form.querySelectorAll('input[type=color]')) input.oninput = updatePreview;
    form.onsubmit = event => { event.preventDefault(); globalThis.vixSaveCustomTheme?.({ bg:form.elements.bg.value, panel:form.elements.panel.value, accent:form.elements.accent.value }); renderAppearance(); vixToast('Tema personalizado aplicado.', 'success'); };
    updatePreview(); $('appearance-contrast').onchange = event => { document.body.classList.toggle('vix-contrast', event.target.checked); localStorage.setItem('vix-interface-contrast', String(event.target.checked)); };
  }
